...
VOID IoT_Cust_Ops(VOID)
{
	...
	IoTCustOp.IoTCustWifiSMInit    = IoT_Cust_SM_Init;
...
BOOLEAN load_usr_cfg(VOID)
{
    ...
    if (IoTpAd.flash_rw_buf[FLASH_USR_CFG_PRODUCT_INFO_STORED] == PRODUCT_INFO_STORED)
    {
        ...
        {
            memcpy((IoTpAd.UsrCfg.ConfigInfor + ReadLen), 
        ...
        else
        {
            ...
            while (ReadLen + FlashLen <= CfgLen)
            {
                ...
                memcpy((IoTpAd.UsrCfg.ConfigInfor + ReadLen), IoTpAd.flash_rw_buf, FlashLen);
            ...
            {
                spi_flash_read(u32FlashBase, IoTpAd.flash_rw_buf, FlashLen);
                memcpy((IoTpAd.UsrCfg.ConfigInfor + ReadLen), IoTpAd.flash_rw_buf, CfgLen - ReadLen);
            }
        }
    }
    else
        ...
        else
        {
            ...
            while (ReadLen + FlashLen <= CfgLen)
            {
                memcpy( IoTpAd.flash_rw_buf,    
                        IoTpAd.UsrCfg.ConfigInfor + ReadLen,
                        FlashLen);
...
VOID IoT_Cust_SM_Smnt2(VOID)
{
	/*this is Customer smart connection entry*/
	/*if set callback here, MTK smart connection will be disabled*/

	/*for example*/
	UCHAR  Ssid[MAX_LEN_OF_SSID+1]= "FAST_5F0696";
	UCHAR  Passphase[CIPHER_TEXT_LEN]="lihailei2014";  //= "Passphase1234";
	UCHAR  PMK[CIPHER_TEXT_LEN];        //= "PMK1234";
	memset(PMK, 0 ,CIPHER_TEXT_LEN);
