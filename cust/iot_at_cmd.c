...
void IoT_exec_AT_cmd_TestSwitch(PCHAR* pCmdBuf, INT16 at_cmd_len)
{
    
#ifndef ZC_OFF_LINETEST
	UINT16 argc = 0;
	char *argv[15];
	char *opString = "t:s:a:r:?";
	char opt=0;
	char *endptr;
	...
	while (opt != -1)
	{
		...
		{
    	case 't':
    		g_struZcConfigDb.struSwitchInfo.u32TraceSwitch = simple_strtol(optarg,&endptr,0);
    	    Printf_High("\1\2\3\4set Trace Switch to %d\n",g_struZcConfigDb.struSwitchInfo.u32TraceSwitch);    		
    		break;
        case 's':
            g_struZcConfigDb.struSwitchInfo.u32SecSwitch = simple_strtol(optarg,&endptr,0);
    	    Printf_High("\1\2\3\4set Security Switch to %d\n",g_struZcConfigDb.struSwitchInfo.u32SecSwitch);    		
            break;
        case 'a':
            g_struZcConfigDb.struSwitchInfo.u32ServerAddrConfig = simple_strtol(optarg,&endptr,0);
    	    Printf_High("\1\2\3\4set Addr Switch to %d\n",g_struZcConfigDb.struSwitchInfo.u32ServerAddrConfig);  		
            break;
        case 'r':
            break;
